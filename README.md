...
- **Optimized for use with the LVGL library** Easy to integrate with the <a href="https://github.com/lvgl/lvgl">lvgl library</a>. Blazing fast (tear free) GUI is obtained using partial differential updates.

- **Driver for an XPT2046 touchscreen.** Some ILI9341 screens have an associated resistive touchscreen. The driver can also manage this touchscreen on the same SPI bus. Thus, only one (or two if using the touch interrupt) additional wires are required.
...
The library can work with any SPI bus. Multiple instances of the driver can manage multiple displays on different SPI buses. *A significant speedup is possible when the DC pin from the ILI9341 screen is connected to a hardware CS (chip select) capable pin on the Teensy*... Yes, this requirement may seem weird at first ! In that case, the library will use the SPI FIFO and DMA to their full capabilities which increases the framerate (around 35% faster) while reducing CPU usage (by around 20%).
...
- **Do not connect any other device (apart from the optional XPT2046 touchscreen) on the same SPI bus**.
...
```
The method can be called again to change the buffering mode on the fly. Calling this method without a parameter removes the current internal framebuffer (and the driver switches to the inefficient direct upload mode).
...
The **framerate** on the other hand is the number of times the screen content is actually updated by the driver every second. When pixel upload and pixel refresh are not synchronized, we end up with the screen displaying part of the old and the new frame simultaneously which creates a visual defect called **screen tearing**.

In order to prevent this, the driver keeps track of the refresh times and tries to upload pixels trailing behind the scanline. However, if the upload rate is much slower than the refresh rate, then the scanline will still eventually catch up with the pixels being uploaded and screen tearing will occur. In order to prevent visual artifacts and ensure a stable framerate, the following two conditions should be met:
...
void setup()
  ...
  while(!tft.begin(SPI_WRITE_SPEED, SPI_READ_SPEED)); // initialization
